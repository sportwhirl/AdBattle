...
async function uploadExactPrivateImage(
  bucket: any,
  userId: string,
  path: string,
  bytes: Uint8Array,
  ...
  try {
    const { error } = await bucket.upload(path, bytes, {
      contentType, cacheControl: "60", upsert: false,
    });
    if (!error) return;
  } catch {
    // A transport exception can arrive after Storage committed the object.
  }

  // Paths are deterministic per request. On any uncertain/error response,
  // trust an existing object only after comparing its exact bytes and hash.
  // A mismatch is never overwritten or deleted here.
  try {
    const stored = await loadOwnedImage(bucket, userId, path);
    if (exactBytes(stored, bytes) && await sha256Hex(stored) === expectedHash) return;
  } catch {
    // Missing, unreadable, or invalid objects all fail closed below.
  }
...
function privateDraftResponse(request: Request, body: Record<string, unknown>, status = 200) {
...
async function reconcileDraftCompletion(
  admin: any,
  userId: string,
  requestId: string,
  expected: DraftCompletion,
) {
  let result: any;
  try {
    result = await admin.from("ai_image_draft_requests")
      .select("status,output_path,output_sha256,post_path,post_sha256,post_bytes,post_width,post_height")
      .eq("request_id", requestId).eq("user_id", userId).maybeSingle();
  } catch {
    return "unknown" as const;
  }
  if (result.error) return "unknown" as const;
  const row = result.data;
  if (!row || row.status === "reserved" || row.status === "failed") {
    return "not_completed" as const;
  }
  if (row.status !== "completed") return "unknown" as const;
  return row.output_path === expected.outputPath &&
      row.output_sha256 === expected.outputHash &&
      row.post_path === expected.postPath &&
      row.post_sha256 === expected.postHash &&
      row.post_bytes === expected.postBytes &&
      row.post_width === expected.postWidth &&
      row.post_height === expected.postHeight
    ? "completed" as const
    : "unknown" as const;
}

async function markDraftFailed(admin: any, userId: string, requestId: string) {
  try {
    const result = await admin.from("ai_image_draft_requests")
      .update({ status: "failed", updated_at: new Date().toISOString() })
      .eq("request_id", requestId).eq("user_id", userId).eq("status", "reserved")
      .select("status").maybeSingle();
    if (!result.error && result.data?.status === "failed") return true;
  } catch {
    // The update may have committed before its response was lost. Re-read it.
  }
  try {
    const { data, error } = await admin.from("ai_image_draft_requests")
      .select("status").eq("request_id", requestId).eq("user_id", userId).maybeSingle();
...
Deno.serve(async (request) => {
  ...
  if (record.reservation_status === "completed") {
    ...
    try {
      return privateDraftResponse(request, { request_id: draft.requestId, status: "completed",
  ...
  if (record.reservation_status === "failed") {
    return jsonResponse(request, { error: "GENERATION_FAILED" }, 409);
  }
  ...
  try {
    ...
    await uploadExactPrivateImage(bucket, user.id, path, image.bytes, outputHash,
      image.mime, "PRIVATE_DRAFT_UPLOAD_FAILED");
    await uploadExactPrivateImage(bucket, user.id, postPath, post.bytes, postHash,
      "image/jpeg", "PRIVATE_POST_IMAGE_UPLOAD_FAILED");
    const completion = { outputPath: path, outputHash, postPath, postHash,
      postBytes: post.bytes.byteLength, postWidth: post.width, postHeight: post.height };
    ...
    try {
      ...
      // postgrest-js reports fetch/connection failures as an error object
      // (commonly with an empty code), not necessarily as a rejected promise.
      // Any returned error therefore leaves the write outcome uncertain.
      updateOutcomeUnknown = Boolean(result.error);
    } catch (error) {
      updateError = error;
      updateOutcomeUnknown = true;
    }
    if (updateError || !updated) {
      const authoritative = await reconcileDraftCompletion(admin, user.id, draft.requestId, completion);
      if (authoritative === "completed") {
        completedPath = postPath;
      } else {
        // Delete only after an authoritative read proves no completed row can
        // reference these deterministic objects. Unknown state is retained for
        // reconciliation or later garbage collection.
        if (!updateOutcomeUnknown && authoritative === "not_completed") {
          try {
            const { error: removeError } = await bucket.remove([path, postPath]);
            if (removeError) console.error("generate-ai-image cleanup failed");
          } catch {
            console.error("generate-ai-image cleanup failed");
          }
        }
        // A transport exception can happen while the database request is still
        // finishing, so even a momentary reserved read does not make deletion
        // safe. Only a definitive write response plus an authoritative
        // uncommitted row permits cleanup.
        finalizationOutcomeUncertain = updateOutcomeUnknown || authoritative === "unknown";
        throw new Error(finalizationOutcomeUncertain
          ? "DRAFT_FINALIZE_OUTCOME_UNCERTAIN" : "DRAFT_FINALIZE_FAILED");
      }
    } else {
      completedPath = postPath;
    }
    ...
    let failureStateUncertain = finalizationOutcomeUncertain;
    if (!finalizationOutcomeUncertain) {
      failureStateUncertain = !await markDraftFailed(admin, user.id, draft.requestId);
    }
    ...
    const responseError = failureStateUncertain
      ? "GENERATION_OUTCOME_UNCERTAIN"
      : policyHold
      ? "PROMPT_NEEDS_REVIEW"
      : "GENERATION_FAILED";
    return jsonResponse(request, { error: responseError },
      !failureStateUncertain && policyHold ? 422 : 503);
  }
  try {
    return privateDraftResponse(request, { request_id: draft.requestId, status: "completed",
