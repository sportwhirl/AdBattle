...
test("orphan cleanup is owner-restricted while uncertain inserts retain their image", () => {
  assert.match(
    migration,
  ...
  assert.match(
    html,
    /const initiatingUserId = currentUser\.id;[\s\S]*const fileName =\s*`\$\{initiatingUserId\}\/\$\{crypto\.randomUUID\(\)\}\.\$\{extension\}`;/,
  );
  assert.doesNotMatch(
    html,
    /if \(adError\)[\s\S]*?\.from\("ad-images"\)\s*\.remove\(\[fileName\]\)/,
  );
  assert.match(html, /uploaded image was retained for reconciliation/);
});
