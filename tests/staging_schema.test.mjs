...
test('supplied staging schema supports both migrations and real wallet RPCs', async () => {
  const db = new PGlite();
  try {
    await db.exec(`
    ...
    await db.exec(base);
    for (const name of ['20260920_wallet_ledger.sql', '20260921_wallet_safety.sql']) {
