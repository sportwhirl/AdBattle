...
`scripts/animate_stills.py` makes a simple five-second moving ad from **one or two
...
| Transition with two stills | `dissolve`, `cut` | One-second dissolve from two to three seconds, or a cut at 2.5 seconds. |
...
and atomically publishes `full.mp4` (exactly five seconds at 24 fps, at most
5 MiB), `poster.jpg` (at most 100 KiB), and optionally `hover.mp4` (three seconds
at 13 fps, at most 400 KiB). If hover alone exceeds its cap, `poster.jpg` is the
